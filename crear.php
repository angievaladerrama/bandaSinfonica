...
if(!empty($_POST["identificacion"])){
 ...
 $identificacion_validador=false;
  ...
  if(!empty($_POST["apellidos"])&& !is_numeric($_POST["apellidos"]) && !preg_match("/[0-9]/", $_POST["apellidos"])){
    ...
    if(count($error)==0){
      $sql= "INSERT INTO users VALUES(NULL,'{$_POST["identificacion"]}', '{$_POST["nombre"]}', '{$_POST["apellidos"]}', '{$_POST["instrumento"]}', '{$_POST["email"]}', '".sha1($_POST["password"])."', '{$_POST["role"]}','{$_POST["direccion"]}','{$_POST["acudiente"]}','{$_POST["telefonoAcudiente"]}', '{$image}');"; //colocar image
